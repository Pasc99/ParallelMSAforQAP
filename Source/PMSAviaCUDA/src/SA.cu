...
SA::SA() :
	T_curr(0), T_0(0), T_end(0), alpha(0.95),
	num_re(1), dim(100), answer(0), success(0.0),
	num_trial(30), N_state(6) // change default value here
{
	mt.seed(rng());
	// Here uses windows.h to create directory
	// If you are using linux, change it
...
void SA::generate_initX()
{
	...
	while (1)
	{
		...
		// CUDA C++ does not support std::filesystem yet
		// Equivelent to: if (std::filesystem::exists(ts_dir.c_str()))
		DWORD ftyp = GetFileAttributes(ts_dir.c_str());
...
void SA::choose_instance()
{
	...
	while (1) {
		...
		if (str == "0") {
			...
			set_param("tai25a", 20000000, 500, 6250, 1167256);
			break;
		}
		else if (str == "5") {
...
void PrintErrorMsg()
{
	...
	std::cout << "Avialable instances: (0) nug15 (1) nug30 (2) tai25a (3) tai50b\n";
	std::cin.get();
}
